 ...
 */
package org.jackhuang.CU.download;

import org.jackhuang.CU.game.*;
import org.jackhuang.CU.task.Task;
import org.jackhuang.CU.util.Logging;
import org.jackhuang.CU.util.SimpleMultimap;
...
import static org.jackhuang.CU.download.LibraryAnalyzer.LibraryType.*;
...
public class MaintainTask extends Task<Version> {
    ...
    private static Version maintainGameWithCpwModLauncher(GameRepository repository, Version version) {
        ...
        if (libraryAnalyzer.has(OPTIFINE)) {
            Library CUTransformerDiscoveryService = new Library(new Artifact("org.jackhuang.CU", "transformer-discovery-service", "1.0"));
            Optional<Library> optiFine = version.getLibraries().stream().filter(library -> library.is("optifine", "OptiFine")).findAny();
            boolean libraryExisting = version.getLibraries().stream().anyMatch(library -> library.is("org.jackhuang.CU", "transformer-discovery-service"));
            optiFine.ifPresent(library -> {
                builder.addJvmArgument("-DCU.transformer.candidates=${library_directory}/" + library.getPath());
                if (!libraryExisting) builder.addLibrary(CUTransformerDiscoveryService);
                Path libraryPath = repository.getLibraryFile(version, CUTransformerDiscoveryService).toPath();
                try (InputStream input = MaintainTask.class.getResourceAsStream("/assets/game/CUTransformerDiscoveryService-1.0.jar")) {
                    Files.createDirectories(libraryPath.getParent());
                    Files.copy(input, libraryPath, StandardCopyOption.REPLACE_EXISTING);
                } catch (IOException e) {
                    Logging.LOG.log(Level.WARNING, "Unable to unpack CUTransformerDiscoveryService", e);
                }
            });
        }
