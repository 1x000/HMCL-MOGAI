...
public final class CUModpackInstallTask extends Task<Void> {
    ...
    public CUModpackInstallTask(Profile profile, File zipFile, Modpack modpack, String name) {
        ...
        try {
            if (json.exists()) {
                ...
                if (!CUModpackProvider.INSTANCE.getName().equals(config.getType()))
                    throw new IllegalArgumentException("Version " + name + " is not a CU modpack. Cannot update this version.");
            }
        ...
        dependents.add(new MinecraftInstanceTask<>(zipFile, modpack.getEncoding(), Collections.singletonList("/minecraft"), modpack, CUModpackProvider.INSTANCE, modpack.getName(), modpack.getVersion(), repository.getModpackConfiguration(name)).withStage("CU.modpack"));
    }
