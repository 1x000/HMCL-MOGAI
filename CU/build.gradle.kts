...
val isOfficial = System.getenv("CU_SIGNATURE_KEY") != null
...
fun attachSignature(jar: File) {
    val keyLocation = System.getenv("CU_SIGNATURE_KEY")
    if (keyLocation == null) {
    ...
    ZipFile(jar).use { zip ->
        zip.stream()
            ...
            .filter { it.name != "META-INF/CU_signature" }
            .forEach {
    ...
    FileSystems.newFileSystem(URI.create("jar:" + jar.toURI()), emptyMap<String, Any>()).use { zipfs ->
        Files.newOutputStream(zipfs.getPath("META-INF/CU_signature")).use { it.write(signature) }
    }
}
...
tasks.getByName<com.github.jengelman.gradle.plugins.shadow.tasks.ShadowJar>("shadowJar") {
    ...
    manifest {
        attributes(
            ...
            "Main-Class" to "org.jackhuang.CU.Main",
...
val makeExecutables = tasks.create("makeExecutables") {
    dependsOn(tasks.jar)
    doLast {
        createExecutable("exe", "src/main/resources/assets/CUauncher.exe")
        createExecutable("sh", "src/main/resources/assets/CUauncher.sh")
    }
}
