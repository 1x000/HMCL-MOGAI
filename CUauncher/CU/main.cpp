...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  if (ERROR_SUCCESS != MyGetEnvironmentVariable(L"CU_JAVA_OPTS", jvmOptions)) {
  ...
  if (!CUJavaDir.empty()) {
    FindJavaInDirAndLaunchJVM(CUJavaDir, workdir, exeName, jvmOptions);
  }
...
error:
  ...
  if (isWin7OrLater) {
    if (isARM64) {
      downloadLink = L"https://docs.CU.net/downloads/windows/arm64.html";
    } if (isX64) {
      downloadLink = L"https://docs.CU.net/downloads/windows/x86_64.html";
    } else {
      downloadLink = L"https://docs.CU.net/downloads/windows/x86.html";
    }
  } else {
    downloadLink = L"https://docs.CU.net/downloads/java.html";
  }
