...
if ($chinese) {
    $messageLabel.Text = "CU 需要 Java 运行时环境才能正常运行，是否自动下载安装 Java？"
} else {
    $messageLabel.Text = "Running CU requires a Java runtime environment. `nDo you want to download and install Java automatically?"
}
...
do {
    $tempFileName = "CU-java-$(Get-Random).zip"
